...
def get_current_user(
    ...
    Raises HTTP 401 if the token is missing, malformed, or the user no longer exists.
    """
...
def get_db_websocket() -> Generator:
    """Standalone DB session generator for WebSocket endpoints (not a FastAPI Depends chain)."""
    db = SessionLocal()
    try:
        yield db
    finally:
