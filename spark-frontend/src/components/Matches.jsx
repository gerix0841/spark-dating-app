...
const Matches = ({ onChatOpen }) => {
  ...
  useEffect(() => {
    const fetchMatches = async () => {
      try {
        ...
        // Non-critical; empty state is rendered if matches can't be fetched
      } finally {
        setLoading(false);
      }
    };
    ...
    const handleRelationUpdate = (e) => {
      setMatches(prev => prev.filter(m => m.user_id !== e.detail.blockedId));
    };
  ...
  return (
    <div className="max-w-5xl mx-auto px-4">
      ...
      {matches.length === 0 ? (
        <div className="bg-white/5 border border-white/10 rounded-[2.5rem] p-12 text-center text-slate-400">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {matches.map((match, index) => (
            ...
            >
              ...
              {new Date(match.created_at) > new Date(Date.now() - TWENTY_FOUR_HOURS_MS) && (
      ...
      <UserDetailModal
        ...
        onSendMessage={(id) => { setIsModalOpen(false); onChatOpen(id); }}
      />
    </div>
  );
};
